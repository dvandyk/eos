...
namespace eos
{
    ...
    /* Helper functions to create ObservableEntry for a composite observable */
    template <typename Decay_, typename Tuple_, typename ... Args_>
    std::pair<QualifiedName, ObservableEntryPtr> make_composite_observable(const char * name,
            const char * latex,
            ...
            const Options & forced_options = Options{})
    {
        ...
        return std::make_pair(qn, make_concrete_composite_observable_entry(qn, latex, prepare_fn, evaluate_fn, kinematics_names, forced_options));
    }
